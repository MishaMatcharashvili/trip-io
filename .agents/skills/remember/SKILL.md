...
`memory.md` is a handoff note, not a replacement for the project plan. Keep the
plan/progress entry points concise: update the relevant linked reference and its
index link rather than appending long records to the entry point. Before saving memory, reconcile the canonical docs:
...
Follow links from the plan/progress entry points only for the feature being
restored. Detailed open work lives in `context/plans/`; completion evidence lives
in `context/history/`. Do not load unrelated history or every linked reference.
