...
Open `context/ui-registry.md`, the pattern index. Read only the reference for the
component being captured. Detailed entries live in `context/ui/`.

Add the detailed entry to the relevant `context/ui/` reference and a link in
`context/ui-registry.md`. Create a focused reference only when no existing topic
fits. Do not append full pattern tables to the index or overwrite other entries.
...
After the developer confirms, write the agreed baseline to the relevant
`context/ui/` reference and link it from `context/ui-registry.md`. Keep the index
free of full pattern tables. Label the reference clearly:
