...
description: "Installation, components, blocks, forms, theming, and MCP guidance for shadcn/ui in modern Next.js projects using Bun"
keywords: ["shadcn", "ui", "Next.js", "bun", "blocks", "sonner", "forms", "dark-mode", "mcp"]
---

# shadcn/ui Skills

**Status:** Next.js 16 ready  
**Package Manager:** Bun (required — see `packageManager` in the root `package.json` for the pinned version; never run `pnpm` or `npm` here)  
**Official Docs:**  
...
- Use Bun for all commands. `shadcn` is already a root devDependency, so prefer `bun run shadcn` over `bunx`.
- This repo is already scaffolded — `packages/ui/components.json` exists. Do not re-run `init`.
  ```bash
  bunx shadcn@latest init   # only for a brand-new package
  ```
...
- Add components when needed (keeps bundle small):
  ```bash
  bunx shadcn@latest add button card input textarea select
  # add blocks or utilities on demand
  ```
- After adding components, run:
  ```bash
  bun run lint && bun run test:unit
  bun run dev:web            # verify styles render
  ```
...
- Add a block via CLI (preferred to avoid copy/paste drift):
  ```bash
  bunx shadcn@latest add blocks/application-shells/sidebar-02
  ```
...
5. **LLM usage:** Follow [llms.txt](https://ui.shadcn.com/llms.txt) when generating code; prefer Bun commands and Sonner over legacy toast.
