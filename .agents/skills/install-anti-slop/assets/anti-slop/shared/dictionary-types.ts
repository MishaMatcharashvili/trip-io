...
export function classifyWideningTarget(
	...
	if (name === "Record" && isBuiltIn(name, environment)) {
		const keyType = unwrapped.typeArguments?.params[0];
		return keyType === undefined || isBroadMappedKey(keyType, environment, new Map())
			? { kind: "open dictionary" }
			: null;
	}
	const alias = environment.aliases.get(name);
...
function classifyAliasBroadTarget(
	...
	if (name === "Record" && isBuiltIn(name, environment)) {
		const keyType = unwrapped.typeArguments?.params[0];
		return keyType === undefined || isBroadMappedKey(keyType, environment, substitutions)
			? { kind: "open dictionary" }
			: null;
	}
	const alias = environment.aliases.get(name);
