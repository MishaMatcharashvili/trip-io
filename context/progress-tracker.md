...
| Watch pipeline | `world_event`, `trip_watch`, `event_match`, `job` all live. Cron at `/api/cron/{sense-weather,match,drain}` behind `CRON_SECRET`, scheduled in `vercel.json`. 218 tests (`npm test`) |
...
- [ ] Vercel Pro enabled, preview deploys working, `CRON_SECRET` set — the three Phase 3 cron
      handlers are written and scheduled in `vercel.json`, and Hobby cron will reject that schedule
      at deploy time
